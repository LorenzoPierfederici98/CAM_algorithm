...
The CT image is preprocessed with a watershed algorithm in order to extract the lung ROI, starting from a bounding box given by the user with the *extrema* positional argument. Since the ground truth wasn't available, a new one is provided by the Otsu thresholding method from scikit-image which distinguishes foreground and background. The CAM results are compared with the region growing flood algorithm from scikit-image.
